...
def test_parse_field_serialized_list_input():
    kicks = _parse_field(
        "['1|1:0|Juan Antonio Señor', '3|2:1|Víctor Muñoz']",
        team="Spain",
        scored=True,
    )
    assert len(kicks) == 2
    assert [k.kick_number for k in kicks] == [1, 3]
    assert [k.running_score for k in kicks] == ["1-0", "2-1"]
    assert [k.player for k in kicks] == ["Juan Antonio Señor", "Víctor Muñoz"]
